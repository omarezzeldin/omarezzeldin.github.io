...
<t:panelGrid forceId="true" id="cnt1Panel" columns="4" width="100%" rowClasses="row01,row02" cellspacing="0"
    ...
    <t:panelGroup >
        <t:inputText id="iem_code" forceId="true" value="#{pageBeanName.itemCode}" styleClass="filteration_input_text" onkeypress="filterDivInputOnKeyPress(event,this,changeItem,'display_btn_id');" onkeyup="enabelIntegerOnly(this);" >
            <a4j:jsFunction name="changeItem" actionListener="#{pageBeanName.changeItem}" reRender="OperationBar,item_name,linesDetails,errorCodePanelId2,myForm:searchItemsBtn"/>
        </t:inputText>
        <f:verbatim>&nbsp;</f:verbatim>
    ...
    <t:panelGroup forceId="true" id="displayBtnPanel" colspan="4" >
        <t:commandButton type="button" id="display_btn_id" value="#{integrationBundle.show_details}" disabled="#{!pageBeanName.employeeHelper.civilExist}"
                           styleClass="cssButtonSmall" style="margin-right:250px;" onclick="if(validatemyForm()){viewCondLines();}" />
        <a4j:jsFunction name="viewCondLines"  action="#{pageBeanName.loadData}" oncomplete="javascript:changeVisibilityMsg();" reRender="linesDetails,divMsg,dataT_data_pnl,dataT_data_line,emptyListId,paging_panel" />
        
        <h:outputText id="confilctedAllowancesId" value="#{integrationBundle.not_combined_allowances}" styleClass="lable01 nowrap_txt" style="font-weight: bold;margin-right: 10px;" rendered="#{pageBeanName.showAllwancesDiv}"/>                         
        <t:commandButton type="button" id="allowances_btn_id" value="..." disabled="#{!pageBeanName.employeeHelper.civilExist}" style="margin-right: 10px;"
                           styleClass="cssButtonSmall" onclick="if(validatemyForm()){showConfilctAllownces();}" rendered="#{pageBeanName.showAllwancesDiv}" />
        <a4j:jsFunction name="showConfilctAllownces"  action="#{pageBeanName.loadNotCombinedAllwancesDiv}" oncomplete="changeVisibilityDiv(window.blocker,window.customDiv2);" reRender="allowncesPanelGroup1,bttnPGId" />
        <%--<t:commandButton type="button" id="allowances_btn_id" value="..." rendered="#{pageBeanName.showAllwancesDiv}"
                           styleClass="cssButtonSmall" oncom="changeVisibilityDiv(window.blocker,window.customDiv2);" action="#{pageBeanName.loadNotCombinedAllwancesDiv}" />--%>
    </t:panelGroup>
</t:panelGrid>    
