                        ...
                        <htm:td valign="top" bgcolor="#FFFFFF" styleClass="paddingbox">
                           <t:panelGrid align="center">  
                            ...
                            <t:panelGrid align="center" columns="4" border="0" width="50%" forceId="true" id="civil_div_searchPanel">
                             ...
                             <t:panelGroup>
                             <h:commandButton  type="button" id="civil_div_search"  onclick="checkempSearchValidity();" styleClass="ManyToManySearchButton"/>
                             <a4j:jsFunction name="searchEmpListOfVal" oncomplete="settingFocusAfterSearchAtEmpLov();" action="#{pageBeanName.empListOfValues.searchLovDiv}" reRender="civil_div_dataT_data_panel,civil_div_searchPanel,errorMessage,civil_div_paging_panel,civil_div_paging_panel,civil_div_ok"/>
                             
                             </t:panelGroup>
                             <f:verbatim>&nbsp;&nbsp;</f:verbatim>
                             <a4j:commandButton id="civil_div_cancelSearch" action="#{pageBeanName.empListOfValues.cancelSearchLovDiv}" oncomplete="settingFocusAfterSearchAtEmpLov();" reRender="civil_div_ok,civil_div_searchText,civil_div_dataT_data_panel,civil_div_searchPanel,errorMessage,civil_div_paging_panel,civil_div_searchRadioBtn,civil_div_ok"  styleClass="ManyToManyCancelSearchButton" rendered="#{pageBeanName.empListOfValues.searchMode}" />
                             ...
                             <htm:center>
                                 ...
                                 <h:outputText id="error" value="#{globalResources.civil_no_not_less_than_12}" styleClass="errMsg" style="visibility:hidden; width:100%;" />
                             </htm:center>
                             </t:panelGroup>
                            </t:panelGrid>
...
<t:panelGrid border="0" cellpadding="0" cellspacing="0" width="100%">
    ...
    <t:panelGroup forceId="true" id="civil_div_dataT_data_panel" styleClass="emplovDivScroll">
       ...
       <t:column id="check_column" styleClass="standardTable_Header3" width="5%">
        ...
        <t:selectOneRadio styleClass="radioButton_DataTable" id="chk" 
         ...
         <a4j:support event="onclick" actionListener="#{pageBeanName.empListOfValues.selectedRadioButton}" reRender="civil_div_ok"/>
        </t:selectOneRadio>
       </t:column> 
        ...
        <t:panelGrid columns="1" rendered="#{ pageBeanName.empListOfValues.listSize == 0 }" align="center">
            <t:outputText value="#{globalResources.global_noTableResults}" styleClass="msg" rendered="#{!pageBeanName.empListOfValues.cantLocateSession}"/>
...
<t:panelGroup forceId="true" id="civil_div_paging_panel">
        <h:panelGrid id="civil_div_panelGrd_scroller" columns="1" dir="#{shared_util.pageDirection}" styleClass="scroller" width="300px" rendered="#{pageBeanName.empListOfValues.listSize > shared_util.noOfTableRows}" >
          ...
          <t:dataScroller id="civil_div_scroll_1" 
                    ...
                    actionListener="#{pageBeanName.empListOfValues.openLovDiv}">
                <f:facet name="first" >                            
...
<t:panelGrid columns="2" align="center">
 <a4j:commandButton id="civil_div_ok" value="#{globalResources.ok}" action="#{pageBeanName.empListOfValues.closeLovDiv}" styleClass="cssButtonSmall" oncomplete="hideLookUpDiv(window.blocker,window.lovEmpPaging);reInitializeMsgs();" reRender="#{pageBeanName.empListOfValues.renderedDropDown}" binding="#{pageBeanName.empListOfValues.okCommandButton}" disabled="#{empty pageBeanName.empListOfValues.selectedDTOS}" />
 
<t:panelGroup>
    <t:commandButton forceId="true" id="backButtonEmpLovDivPaging" onblur="settingFoucsAtEmpLovDiv();" type="button" styleClass="cssButtonSmall" value="#{globalResources.back}"/>
    <a4j:support action="#{pageBeanName.empListOfValues.hideLovDiv}" event="onclick"  oncomplete="hideLookUpDiv(window.blocker,window.lovEmpPaging);if(typeof(setFocusAfterCloseEmpLovDiv)!='undefined'){setFocusAfterCloseEmpLovDiv()};reInitializeMsgs();" reRender="divSearch"/>
</t:panelGroup>
...
<t:inputText styleClass="fakeInputForFocus"/>
...
<t:saveState id="emplovresetDivAfterClose" value="#{pageBeanName.empListOfValues.resetDivAfterClose}"/>
...
<script type="text/javascript">
    ...
    function checkempSearchValidity()
    {
        if(document.getElementById('civil_div_searchText') == null || document.getElementById('civil_div_searchText').value == '')
        {
            document.getElementById('myForm:error').style.visibility = 'visible';
            document.getElementById('myForm:error').innerHTML = '${globalResources.missingField}';
        }
        else if(validateCodeNameSrchParamter('civil_div_searchRadioBtn','civil_div_searchText','','errorMessage'))
        {
            document.getElementById('myForm:error').style.visibility = 'hidden';
            searchEmpListOfVal();
        }
        else{
            document.getElementById('myForm:error').style.visibility = 'visible';
            document.getElementById('myForm:error').innerHTML = '${globalResources.civil_no_not_less_than_12}';
        }
    
    }
    
    function reInitializeMsgs(){
        document.getElementById('myForm:error').style.visibility = 'hidden';
    }
