    ...
    <htm:tr>
        <htm:td align="center">
            <htm:table border="0" width="99%" id="emplovtable5" cellspacing="0" cellpadding="0">
                <htm:tr>
                    <htm:td valign="top"
                            style="background-image: url('${shared_util.contextPath}/app/media/images/g-r.gif'); background-repeat: repeat-y"
                            styleClass="grightbox"></htm:td>
                    <htm:td valign="top" bgcolor="#FFFFFF" styleClass="paddingbox">
                        <t:panelGrid align="center">
                            <t:selectOneRadio forceId="true" id="civil_div_searchRadioBtn"
                                              value="#{pageBeanName.empListOfValues.searchTyp}" styleClass="divtext"
                                              onblur="settingFoucsAtEmpLovDiv();">
                        ...
                        <t:panelGrid align="center" columns="4" border="0" width="50%" forceId="true"
                                     id="civil_div_searchPanel">
                            <t:inputText onkeypress="FireButtonClick('myForm:civil_div_search');" forceId="true"
                                         id="civil_div_searchText" value="#{pageBeanName.empListOfValues.searchQuery}"
                                         styleClass="textboxmedium"/>
                            <t:panelGroup>
                                <h:commandButton type="button" id="civil_div_search"
                                                 onclick="if(checkRadio_CivilId('civil_div_searchRadioBtn','civil_div_searchText','#{globalResources.civil_no_not_less_than_12}')){ searchEmpListOfVal();}"
                                                 styleClass="ManyToManySearchButton"/>
                                <a4j:jsFunction name="searchEmpListOfVal"
                                                oncomplete="document.getElementById('myForm:civil_div_cancelSearch').focus();"
                                                action="#{pageBeanName.empListOfValues.searchLovDiv}"
                                                reRender="civil_div_dataT_data_panel,civil_div_searchPanel,errorMessage,civil_div_paging_panel,civil_div_paging_panel,civil_div_ok"/>
                            </t:panelGroup>
                            <f:verbatim>&nbsp;&nbsp;</f:verbatim>
                            <a4j:commandButton id="civil_div_cancelSearch"
                                               action="#{pageBeanName.empListOfValues.cancelSearchLovDiv}"
                                               reRender="civil_div_ok,civil_div_searchText,civil_div_dataT_data_panel,civil_div_searchPanel,errorMessage,civil_div_paging_panel,civil_div_searchRadioBtn,civil_div_ok"
                                               styleClass="ManyToManyCancelSearchButton"
                                               rendered="#{pageBeanName.empListOfValues.searchMode}"/>
                            <t:outputText value=" " escape="false"
                                          rendered="#{!pageBeanName.empListOfValues.searchMode}"/>
                            <t:panelGroup colspan="4">
                                <htm:center>
                                    <t:outputText value="#{pageBeanName.empListOfValues.errorMsgValue}" forceId="true"
                                                  id="errorMessage" styleClass="errMsgNoHeight"/>
                                </htm:center>
                            </t:panelGroup>
                        </t:panelGrid>
                    </htm:td>
            ...
            <t:panelGrid border="0" cellpadding="0" cellspacing="0" width="100%">
                <t:panelGroup forceId="true" id="civil_div_dataT_data_panel" styleClass="lovDivScroll5"
                    ...
                    <t:dataTable id="civil_div_dataT_data" var="list"
                                 value="#{pageBeanName.empListOfValues.usingPaging ? pageBeanName.empListOfValues.pagingBean.myPagedDataModel : pageBeanName.empListOfValues.myTableData}"
                                 rowStyleClass="#{ pageBeanName.empListOfValues.selected ? 'standardTable_RowHighlighted' : null}"
                                 forceIdIndexFormula="#{list.code.key}" rows="#{shared_util.noOfTableRows}"
                                 rowIndexVar="index" renderedIfEmpty="false"
                                 binding="#{pageBeanName.empListOfValues.myDataTable}"
                                 ...
                                 sortColumn="#{pageBeanName.empListOfValues.sortColumn}"
                                 sortAscending="#{pageBeanName.empListOfValues.ascending}">
                        <t:column id="check_column" styleClass="standardTable_Header3" width="5%">
                            ...
                            <t:selectOneRadio styleClass="radioButton_DataTable" id="chk"
                                ...
                                <a4j:support event="onclick"
                                             actionListener="#{pageBeanName.empListOfValues.selectedRadioButton}"
                                             reRender="civil_div_ok"/>
                            </t:selectOneRadio>
                        </t:column>
                        <t:column sortable="false" width="20%">
                            <f:facet name="header">
                                <h:outputText id="sort_code" forceId="true" styleClass="headerLink"
                    ...
                    <t:panelGrid columns="1" rendered="#{ pageBeanName.empListOfValues.listSize == 0 }" align="center">
                        <t:outputText value="#{globalResources.global_noTableResults}" styleClass="msg msg_no_icon"
                                      rendered="#{!pageBeanName.empListOfValues.cantLocateSession}"/>
...
<t:panelGroup forceId="true" id="civil_div_paging_panel">
    <h:panelGrid id="civil_div_panelGrd_scroller" columns="1" dir="#{shared_util.pageDirection}" styleClass="scroller"
                 width="300px" rendered="#{pageBeanName.empListOfValues.listSize > shared_util.noOfTableRows}">
        <t:dataScroller id="civil_div_scroll_1" fastStep="5" pageCountVar="pageCount" pageIndexVar="pageIndex"
                        ...
                        renderFacetsIfSinglePage="false" actionListener="#{pageBeanName.empListOfValues.openLovDiv}">
            <f:facet name="first">
...
<t:panelGrid columns="2" align="center">
    <t:commandButton id="civil_div_ok" value="#{globalResources.ok}"
                     action="#{pageBeanName.empListOfValues.closeLovDiv}" styleClass="cssButtonSmall"
                     binding="#{pageBeanName.empListOfValues.okCommandButton}"
                     disabled="#{empty pageBeanName.empListOfValues.selectedDTOS}"/>
    <t:panelGroup>
        <t:commandButton forceId="true" id="backButtonEmpLovDiv" onblur="settingFoucsAtEmpLovDiv();" type="button"
                         styleClass="cssButtonSmall" value="#{globalResources.back}"/>
        <a4j:support action="#{pageBeanName.empListOfValues.hideLovDiv}" event="onclick"
                     oncomplete="hideLookUpDiv(window.blocker,window.lovEmp);if(typeof(setFocusAfterCloseEmpLovDiv)!='undefined'){setFocusAfterCloseEmpLovDiv()};"
                     reRender="divSearch"/>
    </t:panelGroup>
    <%-- by Ashraf Gaber to reset date--%>
    <%-- t:commandButton onblur="settingFoucsAtEmpLovDiv();" forceId="true" id="backButtonEmpLovDiv" type="button"
         value="#{globalResources.back}"
         onclick="hideLookUpDiv(window.blocker,window.lovEmp);if(typeof(setFocusAfterCloseEmpLovDiv)!='undefined'){setFocusAfterCloseEmpLovDiv()};"
