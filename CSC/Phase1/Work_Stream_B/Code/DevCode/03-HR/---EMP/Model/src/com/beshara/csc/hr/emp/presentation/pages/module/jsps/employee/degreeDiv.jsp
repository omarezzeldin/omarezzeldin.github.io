...
<h:panelGrid columns="3" align="center" border="0">
    <t:outputLabel rendered="#{!treeDivBean.enableSearchByCode}" value="#{globalResources.name}" styleClass="treepage-link"/>
    <t:inputText forceId="true" id="searchText"  value="#{treeDivBean.searchQuery}" styleClass="textboxmedium" onkeypress="FireButtonClick('myForm:searchTreeDivButtonID');"/>
    <t:panelGrid columns="2" align="center">
        <a4j:commandButton action="#{treeDivBean.searchTreeFromSpecificChar}" reRender="treeDivList,cancelsearchpanel,noResult,okPanel" oncomplete="focusOnSearchResult();" styleClass="ManyToManySearchButton" id="searchTreeDivButtonID"/>
        <h:panelGroup id="cancelsearchpanel">
            <f:verbatim>&nbsp;</f:verbatim>
            <a4j:commandButton action="#{treeDivBean.cancelSearchTree}" reRender="treeDivList,searchText,cancelsearchpanel,noResult,clientErrorMessage,okPanel,radioTreeDivPanel" rendered="#{treeDivBean.searchMode == true}"
...
<h:panelGrid columns="1"  >
    ...
    <t:panelGrid columns="2" align="center">
        <h:panelGroup id="okPanel">
            <a4j:commandButton id="treeDivOkBtn" binding="#{treeDivBean.okCommandButton}"  value="#{globalResources.ok}" styleClass="cssButtonSmall" actionListener="#{treeDivBean.goActionOk}"
                               oncomplete="hidTreeDiv(null,window.blocker,window.divTree,null);focusAfterBackFromTreeDiv();"/> 
        </h:panelGroup>
        
         <h:panelGroup>
                <t:commandButton forceId="true" id="backButtonTreeDiv" onblur="settingFoucsAtDivAdd();" onclick="backJsFunction(); return false;" styleClass="cssButtonSmall" value="#{globalResources.back}"/>
                <a4j:jsFunction name="backJsFunction" actionListener="#{treeDivBean.goActionBack}" oncomplete="hideLookUpDiv(window.blocker,window.divTree,null,null,null);focusAfterBackFromTreeDiv(); " />
            </h:panelGroup>
    </t:panelGrid>
