...
<t:panelGroup forceId="true" id="dataT_data_panel" rendered="#{pageBeanName.multiOrSingleEmps == '1'}" styleClass="#{(pageBeanName.showOnly ? 'dataT-With-4-row-filter'  : 'dataT-With-5-row-filter')}">
...
<!--rowStyleClass="#{detailBeanName.selected ? 'standardTable_RowHighlighted' : null}"-->
    <t:dataTable id="dataT_data" var="list" renderedIfEmpty="false"
       ...
       <t:column id="minId_column" width="25%">
        ...
        <t:column id="elms_column" width="25%">
            <f:facet name="header">
                <t:panelGroup>
                   <t:outputText id="sort_elms_Detail" value=""/>
                </t:panelGroup>
            </f:facet>
            <t:commandButton id="detail_btn" value="#{regResources.elms_details_page_title}" styleClass="cssButtonSmall" 
                                   action="#{detailBeanName.gotoElmsDetail}"  >
            </t:commandButton> 
             <f:verbatim><br/></f:verbatim>
            <t:outputText id="errorValueMessage" forceId="true" styleClass="errMsg" value="#{list.errorValueMessage}" />
        </t:column>
    ...
    <t:inputHidden id="empCountId" forceId="true" value="#{delegationDecisionEmployeesMaintainBean.countOfAllEmpDecision}"/>
