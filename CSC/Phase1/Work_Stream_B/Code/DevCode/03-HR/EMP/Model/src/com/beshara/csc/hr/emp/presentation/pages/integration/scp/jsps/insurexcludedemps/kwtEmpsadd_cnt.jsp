...
<t:panelGrid id="mainEmpsAddPnl" forceId="true" width="100%">
    ...
    <t:panelGroup forceId="true" id="catPanelID" colspan="3">
        ...
        <t:selectOneMenu forceId="true"
            ...
            <a4j:jsFunction name="changeCategory" actionListener="#{pageBeanName.clearMinData}"
                            reRender="mainDataPanel,scriptGenerator,displayBtnPanel,buttonGrid,empDataPnl,ministryNamePanelId,catPanelID,scriptGeneratorPanel,OperationBar,searchPanelId,radioPanel2,errorCodePanelId2"
    ...
    <t:panelGroup forceId="true" id="searchPanelId" colspan="4">
        ...
        <t:inputText forceId="true" id="minFilterationId" styleClass="filteration_input_text"
            ...
            <a4j:jsFunction name="changeMinistry" actionListener="#{pageBeanName.changeMinistry}"
                            reRender="mainDataPanel,scriptGenerator,displayBtnPanel,buttonGrid,empDataPnl,radioPanel2,ministryNamePanelId, OperationBar,ministryName,errorCodePanelId2"/>
        </t:inputText>
    ...
    <h:outputText value="#{intgBundle.civil_id}" styleClass="divtext"/>
        <t:panelGroup colspan="3" id="empDataPnl" forceId="true" >
            <t:inputText onkeypress="FireButtonClickOldStyle(event,'filterByCivilButton')" readonly="#{pageBeanName.civilExist}"
                         ...
                         onkeyup="enabelIntegerOnly(this);" styleClass="textbox"/>
            <t:outputLabel value="*" styleClass="mandatoryAsterisk"/>
            <f:verbatim>&nbsp;</f:verbatim>
            <t:panelGroup>
                <t:commandButton forceId="true" id="filterByCivilButton" type="button" disabled="#{empty pageBeanName.selectedMiniName}"
                                 value="#{globalResources.Available}" styleClass="cssButtonSmall"
                                 onclick="clearMsgsXXX();if(validatemyForm()){searchLines();}" rendered="#{!pageBeanName.civilExist}"/>
            ...
            <a4j:commandButton type="button" id="Search" styleClass="cssButtonSmall"
                               ...
                               disabled="#{pageBeanName.civilExist || empty pageBeanName.selectedMiniName}"/>
            <f:verbatim><br/></f:verbatim>
            ...
            <%--<c2:customValidator componentToValidate="CivilIdAdd" display="dynamic" 
                                errorMessage="#{globalResources.civil_no_not_less_than_12}" highlight="false" 
                                function="checkCivilLength" />--%>
        ...
        <t:panelGroup colspan="3">
            <t:selectOneMenu forceId="true" id="exceptionType" styleClass="Dropdownbox" disabled="#{!pageBeanName.civilExist}"
        ...
        <t:panelGroup colspan="3">
            <t:selectOneMenu forceId="true" id="salElemGuide" styleClass="Dropdownbox" disabled="#{!pageBeanName.civilExist}"
        ...
        <t:panelGroup>
            <t:selectOneMenu forceId="true" id="date_from_monthMenu" styleClass="textboxsmall2"
                             value="#{pageBeanName.fromMonth}" disabled="#{!pageBeanName.civilExist}">
        ...
        <t:panelGroup>
            <t:selectOneMenu forceId="true" id="date_from_yearMenu" styleClass="textboxsmall2"
                             value="#{pageBeanName.fromYear}" disabled="#{!pageBeanName.civilExist}">
                <f:selectItem itemLabel="#{intgBundle.select}" itemValue=""/>
        ...
        <t:panelGroup>
            <t:selectOneMenu forceId="true" id="date_to_monthMenu" styleClass="textboxsmall2"
                             value="#{pageBeanName.toMonth}" disabled="#{!pageBeanName.civilExist}">
                <f:selectItem itemLabel="#{intgBundle.select}" itemValue=""/>
                <t:selectItems value="#{pageBeanName.months}" itemLabel="#{months.name}" itemValue="#{months.code.key}"
                               var="months"/>
            </t:selectOneMenu>
        ...
        <t:panelGroup>
            <t:selectOneMenu forceId="true" id="date_to_yearMenu" styleClass="textboxsmall2"
                             value="#{pageBeanName.toYear}" style="margin-right: 20px;" disabled="#{!pageBeanName.civilExist}">
                <f:selectItem itemLabel="#{intgBundle.select}" itemValue=""/>
                <t:selectItems value="#{pageBeanName.years}" itemLabel="#{years.code.key}" itemValue="#{years.code.key}"
                               var="years"/>
            </t:selectOneMenu>
        ...
        <t:panelGroup colspan="3">
            <t:inputText styleClass="textbox" forceId="true" onkeyup="enabelFloatOnly(this);" id="excepRatio"
                         value="#{pageBeanName.exceptionRatio}" disabled="#{!pageBeanName.civilExist}"/>
            <t:outputLabel value="*" styleClass="mandatoryAsterisk"/>
...
<script language="javascript" type="text/javascript"><!--
  function saveInsurExc() {
    var isValid = validatemyForm();
    //alert("saveInsurExc isValid = "+isValid);
    if(isValid){saveException();} else {return false;}
  }
  ...
  function clearMsgsXXX() {
    ...
    function checkCivilLength() {
      //alert('numberId = '+numberId+" , digitsCount = "+digitsCount);
      var numberToValidate = document.getElementById('CivilIdAdd').value;
      //const len = Math.ceil(Math.log10(numberToValidate + 1));
      //var len2 = Math.ceil(Math.log(numberToValidate + 1) / Math.LN10);
      var len3 = numberToValidate.toString().length;
      const digitsCount = 12;
      //alert(numberToValidate/*+" : len = "+len+" : len2 = "+len2*/+" : len3 = "+len3+" , digitsCount = "+digitsCount + " : " + (len3 == digitsCount));
      if (digitsCount != null && digitsCount != '' && len3 != null && len3 == digitsCount) {
        return true;
      }
      return false;
    }
