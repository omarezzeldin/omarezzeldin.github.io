                ...
                <t:panelGrid columns="1" width="100%">
                    <t:panelGroup forceId="true" id="divDeleteAlert" styleClass="delDivScroll">
                        <t:dataTable id="dataT_Delete" var="list" value="#{detailBeanName.selectedCurrentDetails}" preserveDataModel="false" renderedIfEmpty="false" footerClass="grid-footer" sortable="false"
                            ...
                            <t:column width="25%" defaultSorted="true" rendered="#{pageBeanName.multiOrSingleEmps == '1'}">
                                <f:facet name="header">
                                    <h:outputText id="header_code"  value="#{regResources.civil_id}"/>
                                </f:facet>
                                <h:outputText id="lck1"  value="#{list.citizensResidentsDTO.code.key}"/>
                            </t:column>
                            <t:column width="75%" defaultSorted="true" rendered="#{pageBeanName.multiOrSingleEmps == '1'}">
                                <f:facet name="header">
                             ...
                             <t:column width="25%" defaultSorted="true" rendered="#{pageBeanName.multiOrSingleEmps == '0'}">
                                <f:facet name="header">
                                    <h:outputText id="header_elmguideCode"  value="#{regResources.regulation_subjects_symbol}"/>
                                </f:facet>
                                <h:outputText id="elmguideCode"  value="#{list.code.key}"/>
                            </t:column>
                            <t:column width="75%" defaultSorted="true" rendered="#{pageBeanName.multiOrSingleEmps == '0'}">
                                <f:facet name="header">
                                    <h:outputText id="header_elmguideName" value="#{regResources.section}"/>
                                </f:facet>
                                <h:outputText id="elmguideName" value="#{list.name}"/>
                            </t:column>
                        </t:dataTable>
                    </t:panelGroup>
                    <t:panelGrid columns="2" align="center">
                        <a4j:commandButton id="del_ok_decision_btn" onblur="document.getElementById('CancelButtonDelAlertDiv').focus();" value="#{globalResources.ok}" action="#{detailBeanName.generalDelete}" 
                        styleClass="cssButtonSmall" oncomplete="hideLookUpDiv(window.blocker,window.delAlert,null,null);"
                         reRender="salVariedElmGuidesGroupId, dataT_data_panel,paging_panel,OperationBar,navigationpanel,totalValuePnl"/>
