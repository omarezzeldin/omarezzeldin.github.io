...
<h:panelGrid columns="5" id="navigationpanel" >
  ...
  <t:commandButton value="#{globalResources.PreviousButton}"
   ...
   styleClass="cssButtonSmall"  onclick="ignoremyFormValidation();" 
  ...
  <t:commandButton value="#{globalResources.NextButton}" 
    ...
    <%--    --%>
    <t:commandButton 
        value="#{regResources.saveButton}" 
        action="#{pageBeanName.savePartially}" onclick=" if(validatemyForm('empData') && validatemyForm('elmguideData')) { return true;} else return false;"
        styleClass="cssButtonSmall"
        id="SaveButtonManyToMany" 
        forceId="true"         
        rendered="#{pageBeanName.nextNavigationCase==null}" disabled="#{!(detailBeanName.currentListSize > 0) || pageBeanName.showOnly}"/>
    <t:commandButton 
        value="#{ globalResources.FinishButton}" 
        action="#{pageBeanName.finish}" onclick=" if(validatemyForm('empData') && validatemyForm('elmguideData')) { return true;} else return false;"
        styleClass="cssButtonSmall"
 ...
 <h:commandButton value="#{globalResources.CancelButton}" action="#{pageBeanName.back}"   styleClass="cssButtonSmall"   onclick="ignoremyFormValidation();" immediate="true" />
  --%>
<%--
<t:commandButton id="reviewButton" value="#{regResources.revision}" action="#{pageBeanName.review}" styleClass="cssButtonSmall"  onclick="#{pageBeanName.currentStepJSValidation}" forceId="true" rendered="#{pageBeanName.currentStag==2}"/>
--%>

<%--<t:commandButton id="reviewButton" value="#{regResources.revision}" action="#{pageBeanName.review}" styleClass="cssButtonSmall"  onclick="#{pageBeanName.currentStepJSValidation}" forceId="true" rendered="#{pageBeanName.currentStag ==2}" disabled="#{pageBeanName.showOnly}"/>--%>
<%--<t:commandButton value="#{regResources.execute_decision_title}" 
            action="#{pageBeanName.finish}"  
            styleClass="cssButtonSmall" 
            onclick="#{pageBeanName.currentStepJSValidation}"  
 ...
 <t:commandButton  forceId="true" id="BackButtonManyToMany" 
    ...
    styleClass="cssButtonSmall" />
    <%-- immediate="true" onblur="if(typeof(setFocusAtMyFirstElement)!='undefined'){setFocusAtMyFirstElement()};" onclick="ignoremyFormValidation();" --%>
</h:panelGrid>
 <t:inputHidden value="#{detailBeanName.selectedAvailableListSize}" forceId="true" id="selectedAvailableListSize"/> 
 

 <t:saveState value="#{financialDecisionsListBean.disapleButton}"/>    
 <t:saveState value="#{financialDecisionsListBean.decisionSearchDTO}"/>   
 <t:saveState value="#{financialDecisionsListBean.systemSettingCode}"/>
